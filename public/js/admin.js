...
function call(id){ 
  const connection = usersConnections.find((connections)=> connections.socket_id === id) 
...
function sendMessage(id){
  ...
  console.log("Meu socket_id: ", socket.id)
...
socket.on("admin_received_message", async(params)=>{
  console.log(params) 
  const connection = usersConnections.find((userId) => (userId=params.message.user_id))
  const createDiv = document.createElement("div")
  const divMessages = document.getElementById(`allMessages${params.message.user_id}`)
  createDiv.className = "admin_message_client"
